...
import BracketTest2 from "./BracketTest2";
import BracketTest3, { type Match } from "./BracketTest3";
import {
  Dialog,
  DialogClose,
  DialogContent,
...
const stageReducer = (state: StageState, action: Action): StageState => {
  switch (action.type) {
    case "SET_STAGE_NAME":
...
const BracketCreate = () => {
  ...
  const [bracketType, setBracketType] = useState<string>("single");
      ...
      <div className="w-full max-w-[320px] flex flex-col gap-4">
        <div className="h-full flex flex-col">
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <RadioGroupItem
                  ...
                  className="cursor-pointer"
                />
                <Label htmlFor="freepool" className="cursor-pointer">
                  프리포올
                </Label>
              </div>
            </RadioGroup>
          ...
          {isCreateBracket && (
            ...
            >
              {stage.groups.length > 0 ? "조 대진 배정" : "대진 배정"}
            </Button>
          )}
        </div>
      </div>
      {isCreateBracket ? (
        <BracketTest3 stage={stage} dispatch={dispatch} />
      ) : (
